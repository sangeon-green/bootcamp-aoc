...
(def 전역-상태 {:계산된값 0 :실행된행 #{} :현재행 0 :전체명령어 전체명령어})

;;어거지로 짬
(defn 명령어실행 [전역상태]
  "입력된 명령어가 두번 실행되는 경우를 찾는 함수"
  (if (= (:현재행 전역상태) (count (:전체명령어 전역상태)))
    {:모두실행 true :계산된값 (:계산된값 전역상태)}
    (let [현재진행행 (:현재행 전역상태)
          선택된행 (nth (:전체명령어 전역상태) (:현재행 전역상태))]
      (if (= (:실행된행 전역상태) 현재진행행)
...
#dbg
(defn 모두실행되는경우찾기 [전역상태]
  (loop [index 0]
    (if (true? (:모두실행 (nth (iterate 명령어실행 전역상태) index)))
      (nth (iterate 명령어실행 전역상태) index)
      (recur (inc index)))))

(defn 수정된명령어실행 [변경된전체명령어모음]
  ((fn [변경된전체명령어]
         (let [전체명령어가변경된전역상태 (merge 전역-상태 {:전체명령어 변경된전체명령어})]
           (모두실행되는경우찾기 전체명령어가변경된전역상태))) 변경된전체명령어모음))

; Part 2
(->> 모든-jmp를-nop으로-순차적으로-변경
     수정된명령어실행
     map
     :계산된값
)
